...
const ROOT = new URL('./assets/models/lia/', document.baseURI);
...
function buildMaterials(loader) {
  ...
  return function remap(old) {
    ...
    if (name === 'Mtl_eye_cornea') {
      ...
      let alphaTest = 0, side = THREE.FrontSide, roughness = 0.50;
      let opacity = 1.0, depthWrite = true;

      // Material IDs as they are stored in the supplied MAX -> GLB export.
      // Keep the clothing close to the source material metadata: material 02
      // was BLEND with alpha 0.5 and roughness 0.5; shoes used roughness 0.5.
      if (idx === 10) map = maps.body;
      else if (idx === 9) map = maps.head;
      else if (idx === 2) { map = maps.underwear; transparent = true; opacity = 0.50; depthWrite = false; }
      else if (idx === 4) { map = maps.shoes; }
      else if (idx === 11) { map = maps.hair1; alphaMap = maps.hair1a; alphaTest = 0.28; side = THREE.DoubleSide; }
      else if (idx === 12) { map = maps.hair2; alphaMap = maps.hair2a; alphaTest = 0.28; side = THREE.DoubleSide; }
      else if (idx === 13) { map = maps.hair3; alphaMap = maps.hair3a; alphaTest = 0.28; side = THREE.DoubleSide; }
      else if (idx === 16) { map = maps.head; }

      m = new THREE.MeshStandardMaterial({
        map, alphaMap, transparent, opacity, depthWrite, alphaTest, side,
...
function buildSkinnedModel(flat, bounds) {
  ...
  model.userData.liaRig = rig;
  model.userData.liaModel = true;
...
export function updateLiaAnimation(model, opts={}) {
  const rig = model?.userData?.liaRig;
  if (!rig) return false;
...
export async function loadLiaModel() {
  const tl = new THREE.TextureLoader();
  const remapMaterial = buildMaterials(tl);
  const gltf = await new GLTFLoader().loadAsync(new URL('lia.glb', ROOT).href);
  const raw = gltf.scene;

  // Bake the export's nested MAX transforms to a clean Y-up coordinate space.
  const flat = flattenModel(raw, remapMaterial);
  if (!flat.length) throw new Error('Lia GLB contains no renderable meshes');
  let box = boundsOf(flat);
  box = centerAndGround(flat, box);
  const h = box.max.y - box.min.y;
  if (!(h > 0)) throw new Error('Invalid Lia model bounds');
  ...
  wrapper.userData.liaModel = true;
  wrapper.userData.liaRig = rigged.userData.liaRig;
  ...
  wrapper.userData.liaHeight = TARGET_HEIGHT;
  wrapper.userData.rigVersion = 21;

  updateLiaAnimation(wrapper, { state:'idle', time:0, speedNorm:0 });
  return wrapper;
}
